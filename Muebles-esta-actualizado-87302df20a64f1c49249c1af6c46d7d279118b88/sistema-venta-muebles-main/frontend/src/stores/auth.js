...
export const useAuthStore = defineStore('auth', () => {
  ...
  const userEmail = computed(() => user.value?.email || user.value?.correo || '')              // Email del usuario
  ...
  const login = async (credentials, options = {}) => {
    try {
      ...
      const accessToken = data.token || data.access_token || data.authToken || null
      const newRefreshToken = data.refreshToken || data.refresh_token || null
      const usuario = data.usuario || data.user || data.data?.usuario || data.data?.user || data.perfil || null
      ...
      const message = error.response?.data?.mensaje || error.response?.data?.message || 'Error al iniciar sesión'
      toast.error(message)
