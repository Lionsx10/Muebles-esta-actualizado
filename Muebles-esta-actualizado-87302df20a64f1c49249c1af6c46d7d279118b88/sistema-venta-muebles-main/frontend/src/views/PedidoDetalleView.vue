  ...
  <div class="container-custom section-padding">
    <div class="mb-6 flex items-center justify-between">
      <div>
        ...
        <p class="text-gray-600">Pedido #{{ pedido?.numero_pedido || pedido?.id }}</p>
      </div>
      <div class="flex gap-2">
        <span v-if="pedido" class="badge" :class="getEstadoBadgeClass(pedido.estado)">
          {{ getEstadoLabel(pedido.estado) }}
        </span>
        <button v-if="canRequestQuote" class="btn-primary" @click="requestQuote">Solicitar Cotización</button>
      </div>
    </div>
    ...
    <div v-else-if="pedido" class="space-y-6">
      ...
      <div class="card">
        <div class="card-body">
          ...
          <div v-if="pedido.detalles?.length" class="space-y-4">
            <div v-for="item in pedido.detalles" :key="item.id" class="border rounded-lg p-4">
              <div class="flex items-start justify-between">
                <div class="flex items-start gap-4 flex-1">
                  <img :src="item.imagen_url || '/placeholder-furniture.jpg'" :alt="item.descripcion" class="w-16 h-16 object-cover rounded">
                  <div class="font-medium text-gray-900">{{ item.descripcion }}</div>
                </div>
                <div class="text-right">
                  <div class="text-sm text-gray-500">Precio unitario</div>
                  <div class="text-base font-semibold">${{ formatPrice(item.precio_unitario || 0) }}</div>
                </div>
              </div>
              <div class="mt-2 text-sm text-gray-600">
                <div><span class="font-medium">Cantidad:</span> {{ item.cantidad || 1 }}</div>
                <div v-if="item.medidas"><span class="font-medium">Medidas:</span> {{ item.medidas }}</div>
                <div v-if="item.material"><span class="font-medium">Material:</span> {{ item.material }}</div>
                <div v-if="item.color"><span class="font-medium">Color:</span> {{ item.color }}</div>
                <div v-if="item.estilo"><span class="font-medium">Estilo:</span> {{ item.estilo }}</div>
                </div>
            </div>
          </div>
          <div v-else class="text-sm text-gray-600">Sin productos aún.</div>
        </div>
      </div>
      ...
      <div v-if="pedido.estado !== 'cancelado'" class="card">
        <div class="card-body">
          <div class="text-sm text-gray-600 mb-2">Proceso del pedido</div>
...
const total = computed(() => {
  ...
  return detalles.reduce((acc, d) => acc + (d.precio_unitario || 0) * (d.cantidad || 1), 0)
})

const canRequestQuote = computed(() => {
  return ['nuevo', 'borrador'].includes(pedido.value?.estado) && (pedido.value?.detalles?.length || 0) > 0
})
