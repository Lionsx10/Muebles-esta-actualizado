    ...
    <div class="max-w-md w-full space-y-8">
      ...
      <div>
        ...
        <h2 class="mt-6 text-center text-3xl font-extrabold text-gray-900">
          {{ isAdminMode ? 'Iniciar Sesión de Administrador' : 'Iniciar Sesión' }}
        </h2>
        ...
        <div class="mt-4">
          <template v-if="!isAdminMode">
            <p class="text-sm text-gray-600">
              ¿Eres administrador?
              <router-link
                to="/login?admin=1&redirect=/admin/cotizaciones"
            ...
            <p class="mt-1 text-xs text-gray-500">
              Este acceso es exclusivo para cuentas con rol administrador.
            </p>
          </template>
          <template v-else>
            <p class="text-sm text-gray-600">
              Estás en modo <span class="font-semibold">Administración</span>. Solo cuentas con rol administrador podrán iniciar sesión.
            </p>
          </template>
        </div>
      </div>
...
import logoUrl from '@/assets/logo-comercial-hg.svg'
...
// MODO ADMINISTRADOR - Detecta si el login debe restringirse a administradores
const isAdminMode = computed(() => {
  const adminParam = route.query.admin
  return adminParam === '1' || adminParam === 'true'
})
...
const handleLogin = async () => {
  ...
  try {
    // Llamar al store de autenticación para hacer login y evaluar resultado
    ...
    if (result?.success) {
      // En modo admin, validar que el usuario tenga rol administrador
      if (isAdminMode.value && !authStore.isAdmin) {
        errors.general = 'Esta cuenta no tiene permisos de administrador'
        toast.error('Acceso restringido: solo administradores')
        // Limpiar sesión que pudo haberse abierto con rol no admin
        authStore.logout()
        return
      }

      // Mostrar mensaje de éxito solo si el login fue exitoso
      toast.success('¡Bienvenido! Has iniciado sesión correctamente')

      // Redirigir según contexto: admin -> /admin, normal -> redirect o dashboard
      const redirectTo = isAdminMode.value
        ? '/admin/cotizaciones'
        : (router.currentRoute.value.query.redirect || '/dashboard')
      router.push(redirectTo)
    } else {
      // Mostrar error amigable si el login falló
      errors.general = result?.error || 'Error al iniciar sesión. Por favor, inténtalo de nuevo.'
    }
  } catch (error) {
