...
const xanoAuthConfig = {
  baseURL: process.env.XANO_AUTH_URL || process.env.XANO_API_URL || 'https://your-workspace.xano.io/api:version',
  timeout: 10000,
  headers: {
    'Content-Type': 'application/json',
  }
...
xanoAdminAuthAPI.interceptors.request.use(
  (config) => {
    ...
    logger.info('Xano Admin Auth API Request', {
      method: config.method?.toUpperCase(),
      url: config.url,
      hasAuth: !!config.headers.Authorization
    });
  ...
  (error) => {
    logger.error('Error en request a Xano Admin Auth', { error: error.message });
    return Promise.reject(error);
  }
);

xanoAdminAuthAPI.interceptors.response.use(
  (response) => {
    logger.info('Xano Admin Auth API Response', {
      status: response.status,
      url: response.config.url,
      dataSize: JSON.stringify(response.data).length
    });
  ...
  (error) => {
    ...
    logger.error('Error en response de Xano Admin Auth', errorInfo);
    
    if (error.response?.status === 401) {
      error.message = 'Credenciales de autenticación de administrador inválidas';
    }
    return Promise.reject(error);
  }
);
...
const authAdminPost = async (endpoint, data = {}, token = null) => {
  try {
    ...
    const response = await xanoAdminAuthAPI.post(endpoint, data, config);
    return response.data;
  } catch (error) {
    throw error;
  }
...
const authAdminGet = async (endpoint, params = {}, token = null) => {
  try {
    const config = { params };
    if (token) {
      config.headers = { Authorization: token.startsWith('Bearer ') ? token : `Bearer ${token}` };
    }
    const response = await xanoAdminAuthAPI.get(endpoint, config);
    return response.data;
  } catch (error) {
    throw error;
  }
